		...
		for (int i = 5; i >= 0; i--) {
			if (gamePads[i] instanceof GamePad && gamePads[i].getAxisCount() > 0) {
					...
					if (i == 0) {// Can also differentiate by specific port set
						...
						port0.get(port0.size()-1).whenPressed(new CycleDriveCommand("Turning"));//turning speedscale
						port0.add( new JoystickButton(gamePads[i], Xbox.AXISBTN_L));
						port0.get(port0.size()-1).whenPressed(new CycleDriveCommand("Direct"));//turning speedscale
					/*
				...
				if (!(gamePads[i] instanceof Xbox)) {// ability to create entirely separate
						...
						port0.get(port0.size()-1).whenPressed(new CycleDriveCommand("Turning"));
						port0.add( new JoystickButton(gamePads[i], gamePads[i].AXISBTN_L));
						port0.get(port0.size()-1).whenPressed(new CycleDriveCommand("Direct"));
				...
				if (i == 1) {
					...
					if (!(gamePads[i] instanceof Xbox)){
						...
						port1.get(port1.size()-1).whenPressed(new ShootCommand());
				...
				if (i == 2) {
					...
					if (!(gamePads[i] instanceof Xbox)){
						...
						port2.get(port2.size()-1).whenPressed(new ShootCommand());
				...
				if (i == 3) {
					...
					if (!(gamePads[i] instanceof Xbox)){
						...
						port3.get(port3.size()-1).whenPressed(new ShootCommand());
				...
				if (i == 4) {
					...
					if (!(gamePads[i] instanceof Xbox)){
						...
						port4.get(port4.size()-1).whenPressed(new ShootCommand());
